...
def test_rejects_completed_qa_without_structured_result(monkeypatch, tmp_path):
    ...
    task = store.create('Run main-flow QA on the real change', owner='qa1', metadata={'template_stage':'qa','message_type':'QA_RESULT','required_sections':['status','summary','evidence','validation','risk','next_action']})
...
def test_review_approve_requires_persisted_qa_results(monkeypatch, tmp_path):
    ...
    store = TaskStore('demo')
    qa1 = store.create('Run main-flow QA on the real change', owner='qa1', metadata={'template_stage':'qa','message_type':'QA_RESULT'})
    qa2 = store.create('Run edge-case and regression QA on the real change', owner='qa2', metadata={'template_stage':'qa','message_type':'QA_RESULT'})
    qa1.status = TaskStatus.completed
    qa2.status = TaskStatus.completed
    review = store.create('Review code quality, maintainability, and delivery readiness', owner='review1', metadata={'template_stage':'review','message_type':'REVIEW_RESULT','required_sections':['decision','summary','architecture_review','required_fixes','evidence','validation','next_action']})
...
def test_review_approve_accepts_with_persisted_qa_results(monkeypatch, tmp_path):
    ...
    TeamManager.add_member('demo', 'review1', 'review1-id', agent_type='general-purpose')
    store = TaskStore('demo')
    qa1 = store.create('Run main-flow QA on the real change', owner='qa1', metadata={'template_stage':'qa','message_type':'QA_RESULT','qa_result':{'status':'pass'}})
    qa2 = store.create('Run edge-case and regression QA on the real change', owner='qa2', metadata={'template_stage':'qa','message_type':'QA_RESULT','qa_result':{'status':'pass_with_risk'}})
    qa1.status = TaskStatus.completed
    qa2.status = TaskStatus.completed
    review = store.create('Review code quality, maintainability, and delivery readiness', owner='review1', metadata={'template_stage':'review','message_type':'REVIEW_RESULT','required_sections':['decision','summary','architecture_review','required_fixes','evidence','validation','next_action']})
    qa1.blocks.append(review.id)
