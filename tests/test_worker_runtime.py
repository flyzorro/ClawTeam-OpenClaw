...
def test_run_worker_iteration_recovers_setup_result_from_completion_signal_and_persists_runtime_handoff(monkeypatch, tmp_path):
    _seed_team(tmp_path, monkeypatch)
    monkeypatch.setenv("CLAWTEAM_AGENT_NAME", "qa1")
    monkeypatch.setenv("CLAWTEAM_TEAM_NAME", "demo")
    monkeypatch.setenv("CLAWTEAM_AGENT_ID", "qa1-id")
    ...
    claimed = {"execution_id": None}
    remote_head = "a" * 40
    detached_head = remote_head
    detached_worktree = str(tmp_path / "demo-worktree")
    ...
    def fake_run(*args, **kwargs):
        ...
        payload = {
            ...
            "result_payload": {
                ...
                "install": "- python3 -m venv .venv -> created\n- source .venv/bin/activate -> ok",
                "baseline_validation": f"- git ls-remote origin HEAD -> {remote_head}\n- pytest -q -> 12 passed",
                "known_limitations": "- none",
                "next_action": "handoff to dev",
            },
        }
        (session_dir / "clawteam-demo-qa1.completion.json").write_text(json.dumps(payload) + "\n", encoding="utf-8")
        return _Completed(returncode=0, stdout="", stderr="")
    ...
    assert result["taskId"] == task.id
    assert result["recoveredStatus"] == "completed"
    assert result["recoveredFrom"] == "SETUP_RESULT"
    assert result["recoverySource"] == "runtime_completion_envelope"
    ...
    assert updated.status == TaskStatus.completed
    assert updated.description.startswith("SETUP_RESULT\nstatus: completed\nremote_status: confirmed_latest")
    assert updated.metadata["setup_result"]["detached_worktree"] == detached_worktree
    assert updated.metadata["runtime_handoff"]["detached_worktree"] == detached_worktree
    assert updated.metadata["runtime_handoff"]["detached_head"] == detached_head
    assert updated.metadata["runtime_handoff"]["venv_path"] == ".venv"
    assert updated.metadata["runtime_handoff"]["baseline_commands"] == [
        f"git ls-remote origin HEAD -> {remote_head}",
        "pytest -q -> 12 passed",
    ]
